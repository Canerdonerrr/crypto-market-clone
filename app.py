...
st.markdown("""
    ...
    /* Saf HTML Tablo Stilleri (Pyarrow'u tamamen atlatır) */
    .crypto-table {
        width: 100%;
...
# --- BAŞLIK VE ARAYÜZ ---
st.title("⚡ Profesyonel Kripto Para Piyasası Terminali")
st.markdown("---")
...
if data:
    ...
    # --- PURE HTML TABLO (Pyarrow Bağımlılığını %100 Ortadan Kaldırır) ---
    html_table = filtered_df.to_html(classes='crypto-table', index=False, escape=False, float_format=lambda x: f"{x:,.2f}")
